...
% multivariate_decoder_bms, with CV across subjects for picking lambdaa
% TODO untested... meant to use with neurosynth
%
...
filename = sprintf('multivariate_decoder_CV_roiglm%d_%s_%s_orth=%d_standardize=%d_mixed=%d_corr=%d_extent=%d_Num=%d_intercept=%d_method=%s,%s_getnull=%d_zav=%d_pa=%d_us=%d.mat', roi_glmodel, replace(roi_contrast, ' ', '_'), regressor, do_orth, standardize, mixed_effects, clusterFWEcorrect, extent, Num, intercept, method{1}, method{2}, get_null, zscore_across_voxels, predict_abs, use_smooth);
disp(filename);
