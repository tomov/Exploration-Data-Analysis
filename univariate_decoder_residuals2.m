% copy of univariate_decoder_residuals but for correlating residuals from GLM (i.e. res^2 with TU^2)
% TODO dedupe 

function univariate_decoder_residuals2(roi_glmodel, roi_contrast, glmodel, regressor, do_orth, lambda, standardize, mixed_effects, clusterFWEcorrect, extent, Num, intercept, flip_sign, do_CV, get_null, sphere)
...
for c = 1:numel(masks)
    ...
    for s = 1:length(data)
        ...
        % decode regressor
        res = ccnl_get_residuals(EXPT, glmodel, masks{c}, s);
        res = nanmean(res{1}, 2); % average across voxels
        ...
        % on the right, we only take residual from trial onsets (+ 5 sec HRF offset)
        data(s).res(~data(s).bad_runs,c) = res(which_act,:);

        % optionally flip sign
        data(s).res(:,c) = adjust_sign(data(s), data(s).res(:,c), regressor, flip_sign);
