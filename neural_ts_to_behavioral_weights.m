function neural_ts_to_behavioral_weights(glmodel, regressor, contrast)
...
outfile = ['b_to_t_', replace(contrast, ' ', '_'), '_glm', num2str(glmodel), '_', what, '.mat'];
outfile
...
for c = 1:length(region)
    ...
    clear t;
    for s = 1:length(data)
        t(s) = mean(ccnl_get_tmap(EXPT, glmodel, regressor, c_vox, s));
    end
    all_t{c} = t;
