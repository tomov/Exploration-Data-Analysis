...
groupMembership = nan([nGroups nBarsPerGroup]);
...
if isrow(data)
    ...
    % This would be interpreted as a group at a single x value
    % bar() does not like this.
    %
    % We must transform this into a single column vector.
    % warning('barerrorbar:badInputs','vector input ambiguously specified as a row vector. To avoid this message specify a column vector')
    data = data.';
    if isscalar(barInputs{1})
        barInputs{1} = 1:numel(data);
    end
end
