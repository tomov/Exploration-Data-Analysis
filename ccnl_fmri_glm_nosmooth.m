function ccnl_fmri_glm_nosmooth(EXPT,model,subjects,fake)
    ...
    if ~isdir(EXPT_modeldir); mkdir(EXPT_modeldir); end
    if ~isdir(fullfile(EXPT_modeldir,['model',num2str(model)])); mkdir(fullfile(EXPT_modeldir,['model',num2str(model)])); end
