...
echo ---------------- >> jobs.txt
echo --- Running badre_2012_activations_analysis lambda = 1, p_ax, r_ax  >> jobs.txt
echo ---------------- >> jobs.txt
...
#sbatch_output=`sbatch -p ncf --mem 50000 -t 1-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="matlab -nodisplay -nosplash -nojvm -r $'badre_2012_activations_analysis(21, 3);exit'"`
#sbatch_output_split=($sbatch_output)
#job_id=${sbatch_output_split[3]}
#echo badre_2012_activations_analysis.sh glm 21 normalize=3: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
#

sbatch_output=`sbatch -p ncf --mem 50000 -t 1-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="matlab -nodisplay -nosplash -nojvm -r $'badre_2012_activations_analysis(21, 4);exit'"`
sbatch_output_split=($sbatch_output)
job_id=${sbatch_output_split[3]}
echo badre_2012_activations_analysis.sh glm 21 normalize=4: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
