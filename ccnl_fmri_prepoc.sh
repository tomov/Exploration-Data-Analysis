...
echo ---------------- >> jobs.txt
echo --- Running ccnl_fmri_preproc for subjects ${subj_arg} in parallel >> jobs.txt
echo ---------------- >> jobs.txt
...
for subj in ${subjects[*]}; do
    ...
    # send the job to NCF
    #
    sbatch_output=`sbatch -p ncf --mem 50000 -t 1-15:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="matlab -nodisplay -nosplash -nojvm -r $'ccnl_fmri_preproc(exploration_expt(),  [$subj]);exit'"`
    # for local testing
