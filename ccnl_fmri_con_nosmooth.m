function ccnl_fmri_con_nosmooth(EXPT,model,contrasts,subjects)
    
    % Construct contrasts and perform group-level analysis.
    %
    % USAGE: ccnl_fmri_con_nosmooth(EXPT,model,contrasts,[subjects])
    ...
    %% save contrasts
    modeldir0 = fullfile(EXPT_modeldir,['model',num2str(model)]);
    save(fullfile(modeldir0,'contrasts'),'contrasts','convec');
    
    %% Group-level analysis
    for j = 1:length(contrasts)
        modeldir = fullfile(EXPT_modeldir,['model',num2str(model)],['con',num2str(j)]);
